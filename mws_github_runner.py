...
Structure your response using these EXACT XML tags (do not modify the tag names):
...
def write_market_context(response_text: str) -> None:
    ...
    if not content:
        ...
        log.warning(
            "Could not extract <mws_market_context> section from response "
...
def send_email(response_text: str) -> None:
    ...
    recommendation = extract_section(response_text, "mws_recommendation") or response_text  # fallback: send full response
